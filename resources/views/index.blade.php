...
@section('content')
            ...
            <div style="width:100%;">
            <button type="submit" class="btn btn-primary" style="float:right;  margin-bottom:10px; font-size:1.5vh; "><svg xmlns="http://www.w3.org/2000/svg" style="margin-right:16px" width="16" height="16" fill="currentColor" class="bi bi-plus-circle-dotted" viewBox="0 0 16 16">
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex; ">
                                ...
                                <form action="{{ route('Gerer.index') }}" method="GET">
                                ...
                                @if($name_of_model == 'role')
                                <form action="{{ route('affecteRole' ) }}" method="POST">
                                    @csrf
                                    <button id="btn1" type="submit" class="btn btn-success">Affecter rôle de {{ choose_data($data1->{$column['name']}, $column) }}</button>
                                    <input type="hidden" name="role_id" value="{{ $data1->id}}">
                                </form>

                                <form action="{{ route('affecteRole' ) }}" method="POST">
                                    @csrf
                                    <button id="btn1" type="submit" class="btn btn-danger">Retirer rôle de {{ choose_data($data1->{$column['name']}, $column) }}</button>
                                    <input type="hidden" name="role_id" value="{{ $data1->id}}">
                                </form>
                                @endif
                            </div>
