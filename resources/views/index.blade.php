...
<form action="{{ route('Gerer.create') }}" method="GET" style="width:100%;">
    ...
    <div style="width:100%;">
        <button type="submit" class="btn btn-primary" style="float:right;  margin-bottom:15px; font-size:1.5vh; ">
            <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:20px" width="16" height="16" fill="currentColor" class="bi bi-plus-circle-dotted" viewBox="0 0 16 16">
        ...
        <tbody>
                        ...
                        <div style="display:flex; ">
                            ...
                            @if ($name_of_model == 'role')
                                <form action="{{ route('affecterRole') }}" method="POST">
                                    @csrf
                                    <button id="btn1" type="submit" class="btn btn-success">Assign role of
                                ...
                                <form action="{{ route('retirerRole') }}" method="POST">
                                    @csrf
                                    <button id="btn1" type="submit" class="btn btn-danger">Revoke role of
                                        {{ choose_data($data->{$column['name']}, $column) }}</button>
                                    <input type="hidden" name="role_id" value="{{ $data->id }}">
                                </form>
                            @endif
            ...
            <td>
                <div style="display:flex; ">
                    ...
                    <form action="{{ route('retirerRole') }}" method="POST">
                        @csrf
                        <button id="btn1" id="refreshButton" type="submit" class="btn btn-danger">Revoke role of
                            {{ choose_data($data->{$column['name']}, $column) }}</button>
                            <input type="hidden" name="role_id" value="{{ $data->id }}">
                    ...
                    <form action="{{ route('affecterRole') }}" method="POST">
                        @csrf
                        <button id="btn1" id="refreshButton" type="submit" class="btn btn-success">Assign role of
                            {{ choose_data($data->{$column['name']}, $column) }}</button>
                        <input type="hidden" name="role_id" value="{{ $data->id }}">
                        <input type="hidden" name="name_of_model" value="{{ $name_of_model }}">
                        <input type="hidden" name="user_id" value="{{ $user->id }}">
