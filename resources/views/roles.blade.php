...
@section('content')
    ...
    <form action="{{ route('roles.create') }}" method="GET" style="width:100%;">
        ...
        <h2 style="font-family: 'Noto Sans TC', sans-serif;  margin:5px">Liste roles</h2>
    </form>
        ...
        <tbody>
            @foreach ($roles as $role)
                <tr>
                    ...
                    <td>{{ $role->name }}</td>
                    <td>
                        <a href="{{ route('roles.edit', $role->id) }}" class="btn btn-sm btn-primary"><svg
                        ...
                        <form id="permission-form" action="{{ route('AllPermission', $role->id) }}" method="GET"
                            class="d-inline">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-warning"><svg xmlns="http://www.w3.org/2000/svg"
