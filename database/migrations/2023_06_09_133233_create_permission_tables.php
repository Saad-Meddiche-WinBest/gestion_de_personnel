...
class CreatePermissionTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create($tableNames['roles'], function (Blueprint $table) use ($teams, $columnNames) {
            $table->bigIncrements('id')->comment('grp2-number'); // role id
            if ($teams || config('permission.testing')) { // permission.testing is a fix for sqlite testing
                $table->unsignedBigInteger($columnNames['team_foreign_key'])->nullable();
                $table->index($columnNames['team_foreign_key'], 'roles_team_foreign_key_index');
            }
            $table->string('name')->comment('grp1-text');       // For MySQL 8.0 use string('name', 125);
            $table->string('guard_name')->comment('grp2-text'); // For MySQL 8.0 use string('guard_name', 125);
            $table->timestamps();
